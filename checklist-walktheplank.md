# WalkThePlank v2.8.3-028 beta and event checklist

This is the mandatory human/server acceptance plan for `1MB-WalkThePlank-v2.8.3-028-j25-26.2.jar`.
...
| --- | --- |
| Semantic version/build | `2.8.3-028` |
| Expected filename | `1MB-WalkThePlank-v2.8.3-028-j25-26.2.jar` |
| Git commit | Full clean commit embedded in the final JAR; record in a new annotated build-028 RC tag and operator release archive |
...
| Automated test result/count | 301 tests across 75 classes; zero failures, errors, or skips before clean-commit freeze |
| `verifyReleaseJar` result | Require PASS on two clean builds with byte-identical outputs |
| Test-server sync result | Require staged SHA-256 to match and only build 028 to be active |

Build 010 completed its controlled scenarios, reproducibility/freeze, standalone/full-stack Paper smoke, live-copy schema-v3 preservation, and test-server synchronization and is tagged `v2.4.1-010-rc.1`. Builds 011–026 culminated in successful gameplay, seasonal appearance, event controls, and paged GUI status. Build 027 added bounded dot-prefixed Floodgate identity metadata and schema v4. Build 028 updates the maintained server/tooling and raises the exact API/runtime floor to Paper 26.2 build 84 stable without a schema change. Repeat every automated candidate gate plus live-copy preservation, Bedrock identity/gameplay, event-command, GUI, and sound matrices against the final clean build-028 artifact.
...
Build 010's persistent live-copy smoke preserved 100 rows, 100 UUIDs, score sum 4256, maximum score 149, the unchanged Classic top ten, and `PRAGMA quick_check = ok`. Build 027 subsequently migrated the database to schema v4. Build 028 has no database-schema change, but repeat every preservation check below against its frozen candidate. Historical evidence is not a sign-off.

## 1. Source and release integrity

- [x] Confirm `gradle.properties` says version `2.8.3`, build `028`, Java `25`, Paper `26.2`, minimum build `84`, and exact Paper API `26.2.build.84-stable`.
...
- [x] Record the current build-028 source result and count: 301 tests across 75 classes; zero failures, errors, or skips. Repeat after the clean-commit freeze.
...
- [ ] Confirm embedded `plugin.yml` identifies v2.8.3 build 028 and Paper 26.2; `build-info.properties` and the manifest additionally identify Java 25, exact API `26.2.build.84-stable`, explicit minimum build 84, and the exact artifact name.
...
- [ ] Require these two test-only files beneath `build/scenario-artifacts/`:
  ...
  ```text
  TEST-ONLY-1MB-WalkThePlank-ScenarioHarness-v2.8.3-028.jar
  TEST-ONLY-1MB-WalkThePlank-v2.8.3-028-Failpoints.jar
  ```

- [ ] Require the production file to remain `build/libs/1MB-WalkThePlank-v2.8.3-028-j25-26.2.jar`; prove the instrumented file has a different path and checksum and did not replace it.
...
## 3. Schema-v4 migration, backup retention, Floodgate metadata, categories, preferences, and UUID preservation
...
- [ ] Require a startup log naming a verified sibling backup like `database.db.pre-migration-v4-<epoch>.sqlite`.
- [ ] Confirm the backup exists, is non-empty, and independently returns `PRAGMA quick_check = ok`.
- [ ] Require `PRAGMA user_version = 4` after migration.
- [ ] Confirm `schema_migrations` records schema versions 1, 2, 3, and 4.
...
- [ ] Join as the reproduced Floodgate identity `.Fedecito2579` (`00000000-0000-0000-0009-01f7f208c5b7`), require no join-event exception, complete a score write, and confirm the UUID owns the row while the dot-prefixed name is only its last-known display metadata.
- [ ] Confirm Java names still obey `[A-Za-z0-9_]{3,16}` and the only added form is one leading dot plus 1–16 ASCII letters/digits/underscores. Reject formatting, Unicode, whitespace, controls, multiple/suffix dots, operators, and other punctuation through both repository and direct-SQL fixtures.
...
- [ ] Leave `event.enabled: false`, reload, and require `/walk`, `/walk play`, walk-in activation, `/walk queue join`, and `/walk queue ready` to refuse participation with the translated closed-event message. Require any retained queue to drain, `%infinityparkour_event_enabled%` to be `false`, and available arenas to publish as zero while stats, top, info, validation, Doctor, and staff configuration remain available.
- [ ] Run `/walk admin event status`, then `/walk admin event enable`. Require an atomic `event.enabled: true` disk commit, complete validation, safe reload, matching runtime/placeholder/status card, and success only after post-commit verification. Restart and require the enabled value to persist.
- [ ] Run `/walk admin event disable` during an active disposable run and with queued/pending work. Require safe restoration/drain, atomic `event.enabled: false`, matching runtime/disk state, no reward, no orphaned work, and closed participation without restarting Paper. Also verify `enabled true|false`, `on|off`, and `true|false` aliases.
- [ ] Revoke `infinityparkour.reload`, change the config generation, externally edit the captured file, interrupt commit/reload, and disable the plugin at each stage in disposable tests. Require denial or exact rollback/reconciliation; no stale authorization, partial YAML, silent runtime/disk disagreement, or main-thread file I/O.
- [ ] Editing `event.enabled` followed by `/walk admin reload` remains supported. No date/time scheduling or automatic theme switch may occur.
...
- [ ] Leave `theme.active: custom` in a copy of the historical live configuration. Require `/walk admin validate` and reload to preserve its top-level `JACK_O_LANTERN`, particle visibility/type/count, scoring, reachability, and rewards exactly.
- [ ] Select exactly `default`, `valentine`, `easter`, `summer`, `halloween`, `thanksgiving`, `christmas`, and `anniversary` one at a time. After each validate/reload, require `/walk debug config` to report the exact active theme, resolved block palette, untyped particle/count, and sound-provider summary; start a new run and visually verify that appearance.
- [ ] During each preset run, require exactly the existing current-plus-one-destination platform window, immediate departed-platform cleanup after scoring, and unchanged Classic/Combo/Flawless results. A theme must not affect jump generation or categories.
- [ ] With a particle preset active, repeat one placement using the player's `full`, `reduced`, and `off` preferences. Require the preset count to be reduced/disabled as requested and the stored UUID preference to survive reconnect/restart.
- [ ] For every preset, hear `start`, ordinary `landing`, configured `milestone`, a non-milestone multiple-of-five `combo`, explicit-leave `finish`, and fall/timeout/integrity `failure`. Require only one landing-family cue per jump and silence for admin/reload/shutdown/quit/death/external-teleport cleanup.
- [ ] Repeat direct Paper cues at `volume: 0.8`, `pitch: 1.0`, then at two deliberate valid variants. Repeat with `provider: cmi` on the staged CMI version and require the same volume/pitch intent. With CMI absent/disabled, require one bounded warning and uninterrupted gameplay, never an exception storm.
- [ ] Turn `/walk settings sounds off` for one UUID and require all six theme cues to remain silent across reconnect/restart; turn it on and require cues to return. A theme reload must never overwrite this preference.
- [ ] Reject an unsupported Paper sound, unsafe CMI token containing spaces/command flags/newlines, unknown provider, missing cue mapping, nonboolean `enabled`, volume below 0 or above 1, and pitch below 0.5 or above 2. Require the previous runtime generation to remain authoritative.
- [ ] Confirm the tutorial item names the resolved material when the preset contains one to three distinct blocks, and uses the bounded “configured platform” wording for a larger valid palette.
- [ ] Reject an unknown active theme, uppercase/malformed/reserved preset name, empty or 17-block palette, unsafe block, typed/unsupported particle, count outside 0–1000, 33rd preset, malformed mapping, and unknown nested preset/sound key. Require the previously active runtime generation to remain authoritative after a rejected reload.
...
- [ ] Require `InfinityParkour v2.8.3-028` to enable once with the stopped pre-smoke score count preserved. The 2026-07-28 maintained-profile baseline is 118 UUID-owned rows; record a new baseline explicitly if the staging data is refreshed.
...
- [ ] With `infinityparkour.play` but without the default-off `infinityparkour.queue.join`, confirm `/walk queue join` and `/walk queue ready` are absent from client suggestions and fail closed if invoked directly.
- [ ] While an arena is occupied, use `/walk play` and the GUI without `infinityparkour.queue.join`. Require the bounded wait-nearby message, no queue entry/readiness assignment, and no automatic start after the arena becomes free.
- [ ] Once the arena is free, confirm the same player can start normally without `infinityparkour.queue.join`; the permission must control reservation only, not ordinary play.
- [ ] Confirm `/walk queue status` and `/walk queue leave` remain available through `infinityparkour.play` so stale state can be inspected and safely removed.
- [ ] Explicitly grant `infinityparkour.queue.join`; confirm join/readiness suggestions and FIFO reservation work. Revoke it once while waiting and once while ready; require immediate removal during refresh/action revalidation and no reserved start.
- [ ] Confirm `infinityparkour.player` does not inherit `infinityparkour.queue.join`, the admin parent does inherit it, and a remapped `permissions.queueJoin` node is honored at suggestions, GUI, commands, queue refresh, and start execution.
...
- [ ] Inspect the viewer head with no score and without stats permission in separate fixtures. Require the dedicated bounded no-score and no-permission lore; no unresolved `{{placeholder}}` text may appear.
- [ ] Click the barrier at 53 and require only the current WalkThePlank inventory to close. Click the arrow at 52 and require WalkThePlank to close before `/menu` opens; test available, missing, denied and throwing `/menu` handlers and require a bounded player-facing fallback with no stale-session action or unhandled WalkThePlank exception.
...
- [ ] At run start and after every scored landing, require exactly two visible plugin platforms: the current support and one destination. There must never be a third preview platform.
- [ ] On every scored landing, require the departed platform to restore immediately before the next destination appears. Jumping backward must not find a retained plugin platform and must not permit back-and-forth scoring.
...
- [ ] Observe a normal multi-jump run with instrumentation: base and target must be durable before start mutation; the successor must become durable but remain hidden during traversal; landing must revalidate and consume only that exact run/session/platform generation, restore the predecessor, and place one destination; predecessor deletion and directory fsync must complete before its block lease is released.
...
- [ ] Require `WalkThePlank disabled; arena blocks and player state were restored` and no pending recovery/operations/database-write timeout in both build-028 controlled/console-smoke profiles.
...
- [ ] Confirm build 028 is active and all older WalkThePlank/InfinityParkour JARs—including both `TEST-ONLY-` artifacts—are disabled/absent.
...
- [ ] Re-run `PRAGMA quick_check` and compare row count, UUID count, score sum, maximum, and top ten with the stopped pre-smoke snapshot. The 2026-07-28 maintained profile retained 118 UUID rows, score sum 4993, maximum 149, and `quick_check=ok`; the historical 100-row migration baseline remains documented separately above.
...
- [ ] Confirm pending build-028 restoration/player-recovery records are resolved or preserved before starting an older plugin that cannot read them.
...
- [ ] Record every accepted known limitation, especially the Paper 26.2 build-84 minimum runtime floor, external-command non-transactionality, no automatic reward replay, and the requirement for retained-run ownership plus a live-safe destination during hard-kill player recovery.
