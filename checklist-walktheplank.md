# WalkThePlank v2.5.2-019 beta and event checklist

This is the mandatory human/server acceptance plan for `1MB-WalkThePlank-v2.5.2-019-j25-26.2.jar`.
...
| --- | --- |
| Semantic version/build | `2.5.2-019` |
| Expected filename | `1MB-WalkThePlank-v2.5.2-019-j25-26.2.jar` |
| Git commit | Full clean commit embedded in the final JAR; record in a new annotated build-019 RC tag and operator release archive |
...
| Test-server sync result | Require staged SHA-256 to match and only build 019 to be active |

Build 010 completed its controlled scenarios, reproducibility/freeze, standalone/full-stack Paper smoke, live-copy schema-v3 preservation, and test-server synchronization and is tagged `v2.4.1-010-rc.1`. Builds 011–013 corrected the Paper 26.2 movement baseline, durable activation ownership, within-block forward activation, and velocity cancellation loop. The build-013 real-client pass completed five consecutive forward/backward/sprinting/jumping starts and exposed an idempotency gap in late block-cleanup callbacks. Build 014 preserved newer exact ownership and introduced recursively non-italic pastel GUI tooltips. Its follow-up client rehearsal completed six rapid same-arena runs with only the expected bounded reservation/settled messages and no lifecycle failure, quarantine, or blocked next run; the operator also approved the visual treatment of the original three menu icons. Builds 015–016 proofread the Play text and made the platform label configuration-aware. Build 017 added the viewer's personal-stat head and explicit `/menu`/close controls. Build 018 restores the original two-visible-platform invariant after real-player testing exposed a prematurely placed pipelined successor. Build 019 materializes missing modern GUI defaults in memory so the historical live translations cannot crash `/walk`; it does not weaken the tested walking, sprinting, jumping, or sprint-jumping paths. Repeat the automated candidate gates and Paper startup/reload/shutdown checks against the final clean build-019 artifact.

## Known build-003 migration evidence

Build 010's persistent live-copy smoke preserved 100 rows, 100 UUIDs, score sum 4256, maximum score 149, the unchanged Classic top ten, and `PRAGMA quick_check = ok`. Build 019 has no database-schema change, but repeat every preservation check below against its frozen candidate. Historical evidence is not a sign-off.

## 1. Source and release integrity

- [x] Confirm `gradle.properties` says version `2.5.2`, build `019`, Java `25`, Paper `26.2`, and exact Paper API `26.2.build.62-beta`.
...
- [ ] Confirm embedded `plugin.yml`, `build-info.properties`, and manifest all identify v2.5.2 build 019, Java 25, Paper 26.2 build 62 minimum, and the exact artifact name.
...
- [ ] Require these two test-only files beneath `build/scenario-artifacts/`:
  ...
  ```text
  TEST-ONLY-1MB-WalkThePlank-ScenarioHarness-v2.5.2-019.jar
  TEST-ONLY-1MB-WalkThePlank-v2.5.2-019-Failpoints.jar
  ```

- [ ] Require the production file to remain `build/libs/1MB-WalkThePlank-v2.5.2-019-j25-26.2.jar`; prove the instrumented file has a different path and checksum and did not replace it.
...
- [ ] Require `InfinityParkour v2.5.2-019` to enable once with 100 preserved scores.
...
- [ ] Require `WalkThePlank disabled; arena blocks and player state were restored` and no pending recovery/operations/database-write timeout in both build-019 controlled/console-smoke profiles.
...
- [ ] Confirm build 019 is active and all older WalkThePlank/InfinityParkour JARs—including both `TEST-ONLY-` artifacts—are disabled/absent.
...
- [ ] Re-run `PRAGMA quick_check` and the 100-row/top-ten comparison after build-019 console smoke: 100 UUID rows, score sum 4256, maximum 149, unchanged top ten, `quick_check=ok`.
...
- [ ] Confirm pending build-019 restoration/player-recovery records are resolved or preserved before starting an older plugin that cannot read them.
