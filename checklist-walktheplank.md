# WalkThePlank v2.4.5-014 beta and event checklist

This is the mandatory human/server acceptance plan for `1MB-WalkThePlank-v2.4.5-014-j25-26.2.jar`.
...
| --- | --- |
| Semantic version/build | `2.4.5-014` |
| Expected filename | `1MB-WalkThePlank-v2.4.5-014-j25-26.2.jar` |
| Git commit | Full clean commit embedded in the final JAR; record in a new annotated build-014 RC tag and operator release archive |
...
| Test-server sync result | Require staged SHA-256 to match and only build 014 to be active |

Build 010 completed its controlled scenarios, reproducibility/freeze, standalone/full-stack Paper smoke, live-copy schema-v3 preservation, and test-server synchronization and is tagged `v2.4.1-010-rc.1`. Builds 011–013 corrected the Paper 26.2 movement baseline, durable activation ownership, within-block forward activation, and velocity cancellation loop. The build-013 real-client pass then completed five consecutive forward/backward/sprinting/jumping starts without a state refusal or repeating velocity anomaly, while exposing an idempotency gap in late block-cleanup callbacks. Build 014 preserves a newer exact block owner without a false lifecycle failure and introduces recursively non-italic proofread pastel GUI tooltips. Repeat the automated candidate gates and Paper startup/reload/shutdown checks against the final clean build-014 artifact; its tooltip appearance and post-run cleanup still need direct client confirmation.

## Known build-003 migration evidence

Build 010's persistent live-copy smoke preserved 100 rows, 100 UUIDs, score sum 4256, maximum score 149, the unchanged Classic top ten, and `PRAGMA quick_check = ok`. Build 014 has no database-schema change, but repeat every preservation check below against its frozen candidate. Historical evidence is not a sign-off.

## 1. Source and release integrity

- [x] Confirm `gradle.properties` says version `2.4.5`, build `014`, Java `25`, Paper `26.2`, and exact Paper API `26.2.build.62-beta`.
...
- [ ] Confirm embedded `plugin.yml`, `build-info.properties`, and manifest all identify v2.4.5 build 014, Java 25, Paper 26.2 build 62 minimum, and the exact artifact name.
...
- [ ] Require these two test-only files beneath `build/scenario-artifacts/`:
  ...
  ```text
  TEST-ONLY-1MB-WalkThePlank-ScenarioHarness-v2.4.5-014.jar
  TEST-ONLY-1MB-WalkThePlank-v2.4.5-014-Failpoints.jar
  ```

- [ ] Require the production file to remain `build/libs/1MB-WalkThePlank-v2.4.5-014-j25-26.2.jar`; prove the instrumented file has a different path and checksum and did not replace it.
...
- [ ] Require `InfinityParkour v2.4.5-014` to enable once with 100 preserved scores.
...
- [ ] Inspect tutorial, play, and Top 10 tooltips. Require proofread wording; pale-blue names; readable white/soft-gray body text; pink warning/label text; gold click actions; and no italic text on any display-name, lore, blank, dynamic leaderboard, or no-permission line.
...
- [ ] Require `WalkThePlank disabled; arena blocks and player state were restored` and no pending recovery/operations/database-write timeout in both build-014 controlled/console-smoke profiles.
...
- [ ] Start and end at least five rapid consecutive runs in the same arena, including one scored run. Delay old main-thread cleanup callbacks until a newer run leases a reused coordinate; require the stale completion to accept an already released lease or preserve the newer exact owner without a lifecycle failure, world mutation, quarantine, duplicate lease release, or blocked next run.
...
- [ ] Confirm build 014 is active and all older WalkThePlank/InfinityParkour JARs—including both `TEST-ONLY-` artifacts—are disabled/absent.
...
- [ ] Re-run `PRAGMA quick_check` and the 100-row/top-ten comparison after build-014 console smoke: 100 UUID rows, score sum 4256, maximum 149, unchanged top ten, `quick_check=ok`.
...
- [ ] Confirm pending build-014 restoration/player-recovery records are resolved or preserved before starting an older plugin that cannot read them.
