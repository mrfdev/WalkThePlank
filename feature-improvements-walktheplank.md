...
4. [ ] **Future ideas — explicitly shelved, not part of the event release.** Momentum, quests, community plank goals, and a seeded daily challenge with a separate leaderboard are ideas for a later design cycle only. Do not implement or expose them before a separate operator decision and adequate test window. Preserve the imported Classic leaderboard.
...
- Every existing all-time row and the top ten survive upgrades unless an operator deliberately restores a backup. The maintained 2026-07-28 staging baseline contains 118 UUID-owned rows; the original migration fixture contains 100.
...
| Production-isolation proof | **Implemented; clean freeze pending** | Negative scans require no scenario classes, controls, canaries, properties, manifest markers, commands, or 24 failpoint IDs in production; positive controls require their presence in test artifacts so a broken scan cannot pass silently. The development artifact passed both controls; repeat after the clean freeze. |
| Controlled Paper 26.2 profile | **Implemented; clean-candidate rerun pending** | The build-028 development-artifact two-start run passed restart-marker, PlaceholderAPI absent/present, config reload, terminal disable/service removal, fresh-JVM enable, deterministic marker, broad log, port-release, and SQLite quick-check assertions. The automated `config.after_runtime_commit` hard halt also exited 97 and recovered cleanly. Repeat both against the clean build-030 candidate; real-client and the remaining named hard-kill cases stay separate acceptance work. |
...
| Successor durability pipeline | **Implemented; latency/gameplay verification pending** | Durable preparation begins during traversal and remains hidden when ready. An early landing waits; the exact grounded landing revalidates ownership, restores the departed platform and places one destination without async world mutation. Journal deletion and exact lease release complete off-thread. |
...
| Seasonal event presets and sounds | **Implemented; final build-030 matrix pending** | The exact bundled roster is `default`, `valentine`, `easter`, `summer`, `halloween`, `thanksgiving`, `christmas`, and `anniversary`. Each resolves a safe block palette, untyped particle, and six validated Paper/optional-CMI cues. `event.enabled` manually opens/closes participation while the plugin remains loaded. No scheduling exists. |

### Capacity, commands, and operator tooling

| Capability | Status | Notes |
| --- | --- | --- |
| FIFO queue/readiness | **Implemented; multiplayer rehearsal pending** | Join/readiness reservation requires the default-off `infinityparkour.queue.join` leaf. Players without it can start whenever an arena is genuinely free but are never auto-enrolled; they wait and retry. Granted players retain leave/status, cooldown, expiry/reminders, action-bar state, pause with frozen positions/deadlines, resume, and drain. |
...
| Retained-run investigation | **Implemented; role/server rehearsal pending** | Separate query permission; exact status/player UUID/run UUID/arena/season/release filters and half-open start windows capped at 366 days; redacted newest-first 1–100-row output with linked plan status; one-second query rate limit and successful-query audit. Every query also has a mirrored atomic JSON export that requires both investigation and export permissions, revalidates both immediately before off-thread I/O, is limited to 100 rows, binds its basename and SHA-256 into the audit chain, and excludes names, raw commands, operator identity, and absolute paths. |
...
| Capability | Status | Notes |
| --- | --- | --- |
| Nonce/generation GUI sessions | **Implemented; exploit matrix pending** | Owner UUID, random nonce, monotonic generation, exact inventory identity, stale-session invalidation, duplicate-click suppression, one pending action, and immediate permission/state revalidation supplement typed-holder click/drag cancellation and the 750 ms open limiter. The viewer head, `/menu` arrow and close barrier use the same gate; the head reads one immutable score snapshot and the back action executes as the player only after closing WalkThePlank. |
| Literal dynamic components and MiniMessage opt-in | **Implemented; live translation verification pending** | Trusted templates are parsed before dynamic values are inserted literally. Unmarked values retain legacy `&` compatibility; only an explicit `minimessage:` marker opts an operator-controlled template into MiniMessage. Bundled GUI names/lore use the shared pastel palette, every component branch forces italics off, exact historical/previous definitions upgrade in memory, new action sections inherit bundled defaults without a rewrite, and custom definitions remain authoritative. |
...
1. **Qualify and commit the build-030 candidate.** After this uncommitted feature pass is reviewed, commit the exact source, create two byte-identical clean builds, rerun the controlled two-start and automated hard-kill profiles against that exact artifact, then record archive verification, final hash/size/test result, Paper 26.2 build-84-stable-or-newer smoke, retained-run query/export role rehearsal, test-server sync, and plugin-log review. Earlier evidence remains historical.
2. **Repeat schema-v4 preservation on a disposable copy of the current data.** Record the stopped input count instead of assuming the historical 100-row fixture; the maintained 2026-07-28 staging baseline has 118 UUID-owned rows. Compare every Classic UUID/score/top-ten position, verify source/database/automatic backups, exercise retention without touching operator files, prove `_resources` stayed unchanged, and complete a dot-prefixed Floodgate identity/score write owned by its UUID.
...
**Core retained-run queries and bounded bundle export implemented; annotation workflow proposed.** Build 003 introduced a separate investigation permission and redacted commands for recent runs by status, player UUID, exact run UUID, arena, or season, plus one-run inspection with linked reward-plan status. Build 030 exposes exact release and half-open start-time interval syntax capped at 366 days. All results are newest-first and capped at 100, successful queries audit the actor, safe filter, and count, and repeated queries are limited per actor.

Every query form has a mirrored `run export` form. Export requires both the investigation and export leaves, revalidates both after the database query and immediately before operations-worker I/O, and writes one no-overwrite JSON file with atomic no-replace hard-link publication. Its fixed schema contains generator provenance, canonical UUID/status/time query fields, SHA-256 representations of free-form arena/release selectors, and the same redacted result projection; audit filters use the same selector digests. It excludes player names, operator identity, raw reward command content, idempotency material, and filesystem paths. The command reports success only after the same required operations-worker task durably binds the basename and SHA-256 into `run.investigation_export`. Failures use bounded categories, and any post-publication or required-audit ambiguity reports the safe basename and expected digest and tells the operator to inspect the export directory before retrying. Export attempts have a stricter per-actor rate limit than display queries.
...
#### Seeded daily challenge — future idea, shelved

Do not implement this for the current event. If reconsidered in a future design cycle, persist date, seed, timezone policy, and algorithm version so every player receives the same deterministic course across restarts. Keep the daily leaderboard separate and do not use the public seed for security-sensitive randomness.
...
- **Future idea, shelved:** optional WorldGuard-style region validation while retaining native protection as the baseline. No telemetry, bStats, or external metrics integration is planned.
...
The final three gameplay concepts are permanent owner-directed exclusions and must not be
reintroduced into the roadmap.
